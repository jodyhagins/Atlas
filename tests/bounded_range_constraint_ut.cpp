...
TEST_SUITE("Bounded Range Constraint (Half-Open)")
{
    ...
    TEST_CASE("bounded_range constraint - integer - max boundary excluded")
    {
        ...
        CHECK_THROWS_AS(test::HalfOpenPercentage{100}, atlas::ConstraintError);  // Max boundary excluded!
    }

    TEST_CASE("bounded_range constraint - integer - invalid construction")
    {
        CHECK_THROWS_AS(test::HalfOpenPercentage{-1}, atlas::ConstraintError);   // Below min
        CHECK_THROWS_AS(test::HalfOpenPercentage{100}, atlas::ConstraintError);  // At max (excluded)
        CHECK_THROWS_AS(test::HalfOpenPercentage{101}, atlas::ConstraintError);  // Above max
    ...
    TEST_CASE("bounded_range constraint - float - max boundary excluded")
    {
        ...
        CHECK_THROWS_AS(physics::CelsiusRange{100.0}, atlas::ConstraintError);   // Exactly at max
    }

    TEST_CASE("bounded_range constraint - float - invalid construction")
    {
        CHECK_THROWS_AS(physics::CelsiusRange{-0.1}, atlas::ConstraintError);    // Below min
        CHECK_THROWS_AS(physics::CelsiusRange{100.0}, atlas::ConstraintError);   // At max (excluded)
        CHECK_THROWS_AS(physics::CelsiusRange{100.1}, atlas::ConstraintError);   // Above max
    }

    TEST_CASE("bounded_range constraint - construction with out-of-bounds value")
    {
        // Direct construction with out-of-bounds value should throw
        CHECK_THROWS_AS(test::HalfOpenPercentage{100}, atlas::ConstraintError);  // Max excluded
    ...
    TEST_CASE("bounded_range constraint - narrow half-open range")
    {
        ...
        CHECK_THROWS_AS(test::TinyRange{41}, atlas::ConstraintError);  // Below min
        CHECK_THROWS_AS(test::TinyRange{44}, atlas::ConstraintError);  // At max (excluded!)
        CHECK_THROWS_AS(test::TinyRange{45}, atlas::ConstraintError);  // Above max
    }
    ...
    TEST_CASE("bounded_range constraint - negative bounds half-open range")
    {
        ...
        CHECK_THROWS_AS(test::NegativeRange{-11}, atlas::ConstraintError);  // Below min
        CHECK_THROWS_AS(test::NegativeRange{10}, atlas::ConstraintError);   // At max (excluded!)
        CHECK_THROWS_AS(test::NegativeRange{11}, atlas::ConstraintError);   // Above max
    }

    TEST_CASE("bounded_range constraint - exception message shows value and bounds")
    {
        try {
    ...
    TEST_CASE("bounded_range constraint - floating point boundary precision")
    {
        ...
        CHECK_THROWS_AS(physics::CelsiusRange{100.0}, atlas::ConstraintError);  // Exact max (excluded!)

        // Values very close but outside should fail
        CHECK_THROWS_AS(physics::CelsiusRange{-0.000001}, atlas::ConstraintError);
    ...
    TEST_CASE("bounded_range constraint with checked arithmetic - invalid construction")
    {
        CHECK_THROWS_AS(test::BoundedRangeChecked{100}, atlas::ConstraintError);  // At max (excluded)
        CHECK_THROWS_AS(test::BoundedRangeChecked{101}, atlas::ConstraintError);  // Above max
    }

    TEST_CASE("bounded_range constraint with checked arithmetic - overflow throws before constraint")
    {
        test::BoundedRangeChecked a{60};
        test::BoundedRangeChecked b{50};

        // 60 + 50 = 110, which is within uint8_t range, but violates bounded_range constraint
        // This should throw ConstraintError after addition
        CHECK_THROWS_AS(a + b, atlas::ConstraintError);
    }

    TEST_CASE("bounded_range constraint with checked arithmetic - valid operations")
    {
        test::BoundedRangeChecked a{60};
    ...
    TEST_CASE("bounded_range constraint with checked arithmetic - result at boundary")
    {
        test::BoundedRangeChecked a{50};
        test::BoundedRangeChecked b{50};

        // 50 + 50 = 100, which is at the max boundary (excluded in half-open range!)
        CHECK_THROWS_AS(a + b, atlas::ConstraintError);
    }
    ...
    TEST_CASE("bounded_range constraint - celsius range arithmetic")
    {
        ...
        CHECK_THROWS_AS(a * b, atlas::ConstraintError);  // 1250.0 exceeds [0, 100)
    }

    TEST_CASE("bounded_range constraint - celsius range - result at max boundary throws")
    {
        physics::CelsiusRange a{50.0};
    ...
    {
        CHECK_NOTHROW(test::BoundedRangeString{"A"});     // Min boundary (inclusive)
    ...
    TEST_CASE("bounded_range constraint - string - max boundary excluded")
    {
        ...
        CHECK_THROWS_AS(test::BoundedRangeString{"AAAA"}, atlas::ConstraintError);
    }
    ...
    TEST_CASE("bounded_range constraint - string - invalid construction")
    {
        ...
        CHECK_THROWS_AS(test::BoundedRangeString{"AAAA"}, atlas::ConstraintError);  // At max (excluded!)
        CHECK_THROWS_AS(test::BoundedRangeString{"AAAAA"}, atlas::ConstraintError);
        CHECK_THROWS_AS(test::BoundedRangeString{"AAAB"}, atlas::ConstraintError);
    ...
    TEST_CASE("bounded_range constraint - string - exception message shows value and bounds")
    {
        try {
    ...
    TEST_CASE("bounded_range constraint - string - copy and move don't re-check")
    {
