    ...
    TEST_CASE("File Input/Output Functionality")
    {
        ...
        SUBCASE("file mode with multiple hash specializations") {
            ...
            auto input_path = tester.temp_dir_ / "hash_types.txt";
            {
                ...
                input
                    << "description=strong unsigned; ==, no-constexpr-hash\n\n";
                input << "[type]\n";
                input << "kind=struct\n";
                input << "namespace=strings\n";
                input << "name=Label\n";
                input << "description=strong std::string; ==, "
                         "no-constexpr-hash\n";
            }
            ...
            auto notice = output.find("// NOTICE");
            CHECK(notice != std::string::npos);
            auto gen1 = output.find("generated by Atlas");
            REQUIRE(notice != std::string::npos);
            auto gen2 = output.find("generated by Atlas", gen1 + 1);
