...
TEST_CASE("non_negative constraint with checked arithmetic - valid operations")
{
...
TEST_CASE("non_negative constraint with checked arithmetic - zero result is OK")
{
...
TEST_CASE("non_negative constraint with saturating - underflow to zero is OK")
{
    ...
    "non_negative constraint with saturating - overflow stays non-negative")
{
    ...
    // Saturating addition: 200 + 100 saturates to 255, which is non-negative
    CHECK_NOTHROW(a + b);
